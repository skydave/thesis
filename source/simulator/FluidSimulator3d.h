...
struct Particle3d
{
	Particle3d( math::Vec3f _position ) : position(_position), velocity()
	{
	}
	Particle3d( )
	{
...
//protected:
public:
	/// \brief cell makes up the grid which is used to solve the equations
	...
	struct Cell
	{
		...
		Cell                             *getNeighbour( int ri, int rj, int rk ); ///< retrieve the neighbour from given relative coordinates. ri, rj, rk each are -1, 0 or +1
		...
		math::Vec3f                                                     velocity; ///< velocity components (components are assumed to lie on cell-faces)
		math::Vec3f                                               velocityChange; ///< for the FLIP method we need the change in velocity
		math::Vec3f                                                  oldVelocity; ///< to compute the change in velocity we need the velocity of the previous step
		...
		bool                                                    zNeighboursFluid; ///< this bool tells wether the z component of the velocity lies next to a fluid cell

		Cell                                                     *neighbours[26]; ///< references to all 10 neighbouring cells which are used for interpolation (interpolation-neighbours) and simulation
		bool                                     allInterpolationNeighboursExist; ///< this routine tells whether all 7 neighbouring cells for interpolation have been created and are present within the hashtable


		math::Vec3f                                                 tempVelocity; ///< temp variables for the velocity components (used by the solver)
		...
		// sand-fluid extensions...
		bool                                                               rigid; ///< used by the sand simulator to identify rigid cells
		math::Vec3f                                                       center; ///< we need the position of the cell centers for the rigidification
		math::Matrix44f                                             strainTensor; ///< this tensor represents the stress situatiopn for the current cell

		// viscoelastic fluid extensions...
		math::Matrix44f                                                     temp; //
		math::Vec3f                                                   tempVector;
	...
	struct StaticSolid
	{
		virtual bool              occupies( const math::Vec3f &cellCenter )=0; ///< this function is used to indirectly voxelize the obstacle
		...
		virtual void                    check( size_t i, size_t j, size_t k, const math::Vec3f &cellCenter ){}
	...
	///
	struct hash_compare : public stdext::hash_compare<FluidSimulator3d::Cell::Coordinate>
	{
		size_t operator() (const FluidSimulator3d::Cell::Coordinate& key) const;  ///< hash function which returns an hashvalue for the given key
		bool operator() (const FluidSimulator3d::Cell::Coordinate& key1, const FluidSimulator3d::Cell::Coordinate& key2) const; ///< comperator for internal sorting (dont ask me why hash_table is sorting at all)
	};



public:
	FluidSimulator3d();                                                                                ///< constructor
	~FluidSimulator3d();                                                                               ///< destructor

	std::vector<Particle3d>                                                           markerParticles; ///< particles used to track the volume of the fluid in space
...
public:
	...
	float                                                                               getCellSize(); ///< returns the cellsize
	void                                                setDeltaTimeRange( float dtmin, float dtmax ); ///< set the range in which the delta time must be for one simulation step
	void                        setSimulationDomain( const math::Vec3f &min, const math::Vec3f &max ); ///< sets the spatial boundaries of the simulation
	void                                                             setViscosity( float _viscosity ); ///< sets the simple-fluid visosity
	Cell                                                     *getCell( size_t i, size_t j, size_t k ); ///< returns a pointer to the cell which lies at the specified coordinate - if the cell does not exist, null is returned
...
//protected:
	...
	virtual void                                  applyGravity( float dt, float x, float y, float z ); ///< will add dt*math::Vec3f(x,y,z) to the respective velocity components, which neighbour fluid cells
	...
	void sweepUpdateDistances( Cell *cell, int horizontalNeighbourIndex, int verticalNeighbourIndex, int depthNeighbourIndex ); ///< This is the actual distance computation proposed in [Zhao2005]
	...
	void         sweepVelocityUVW( int iStart, int iEnd, int jStart, int jEnd, int kStart, int kEnd );
	...
	float     getInterpolatedVelocityValue( float x, float y, float z, unsigned int componentIndex  ); ///< returns the index-specified trilinear interpolated velocity-component from the grid.
	float       getInterpolatedChangeValue( float x, float y, float z, unsigned int componentIndex  ); ///< returns the index-specified trilinear interpolated velocity-component change from the grid.
	math::Vec3f                                              getVelocity( float x, float y, float z ); ///< This function does look up the velocity within the simulationDomain
	math::Vec3f                                        getVelocityChange( float x, float y, float z ); ///< This function does look up the change in velocity within the simulationDomain (used for FLIP)

	math::Vec3f                         traceParticle( const math::Vec3f &position, float deltaTime ); ///< This method returns the position of the given point at t - deltaTime (used for semi-lagrange style advection)
